  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <div className="flex gap-4 mb-6 border-b border-gray-200 pb-2">
        ...
        {profile.role === "Admin" && (
          <button
            ...
            className={`pb-2 px-2 text-sm font-semibold transition ${
              activeTab === "permissions" ? "text-red-600 border-b-2 border-red-600" : "text-gray-500 hover:text-gray-700"
            }`}
          >
            System Permissions
          </button>
        ...
        <div className="bg-white p-8 shadow-sm border border-gray-100 rounded-2xl">
          <div className="flex items-center gap-6 mb-8 pb-8 border-b border-gray-100">
            ...
            <div>
              ...
              <span className="inline-block mt-1 px-3 py-1 bg-red-50 text-red-600 text-xs font-bold rounded-full">
                {profile.role}
              </span>
            </div>
          </div>
      ...
      {activeTab === "permissions" && (
        <div className="bg-white p-8 shadow-sm border border-gray-100 rounded-2xl max-w-xl">
          <h2 className="text-lg font-bold text-gray-800 mb-6">System Access Configuration</h2>
          <div className="space-y-4 bg-gray-50 p-6 rounded-xl border border-gray-100">
            <div className="flex justify-between items-center">
              <span className="text-sm font-bold text-gray-500 uppercase">Assigned Role</span>
              <span className="font-semibold text-gray-800 bg-white px-3 py-1 rounded-lg border border-gray-200">{profile.role}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-sm font-bold text-gray-500 uppercase">Branch ID</span>
              <span className="font-semibold text-gray-800 bg-white px-3 py-1 rounded-lg border border-gray-200">{profile.branchId || "Global / Head Office"}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-sm font-bold text-gray-500 uppercase">Account Status</span>
              <span className="font-semibold text-green-600 bg-green-50 px-3 py-1 rounded-lg border border-green-100">
                {profile.isActive ? "Active & Verified" : "Suspended"}
              </span>
            </div>
          </div>
        </div>
