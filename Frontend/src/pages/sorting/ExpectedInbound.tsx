...
function ExpectedInboundList() {
    ...
    const filteredOrders = useMemo(() => {
        return orders.filter(x => {
            ...
            // Only Expected Inbound
            return x.status === "Dispatched to Warehouse";
        });
    }, [orders, search]);
    ...
    const expectedCount = filteredOrders.length;

    if (loading) {
        return <div className="p-8 text-blue-600 font-semibold animate-pulse">Loading expected inbound...</div>;
    }

    return (
        <div className="min-h-screen p-8 bg-gradient-to-br from-slate-50 to-blue-50 relative">
            <h1 className="text-4xl font-bold mb-2 text-slate-800">Expected Inbound</h1>
            <p className="text-gray-500 mb-8">Parcels arriving from branches that have not been received yet.</p>
            ...
            <div className="grid md:grid-cols-1 gap-6 mb-8 max-w-sm">
                <div className="bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded-3xl p-6 shadow-lg transform transition hover:scale-105">
                    <Inbox size={32} />
                    <h2 className="mt-3 font-medium opacity-90">Expected Inbound</h2>
                    <p className="text-4xl font-bold tracking-tight">{expectedCount}</p>
                </div>
            </div>
                ...
                <div className="flex items-center gap-4 bg-slate-50 p-2 rounded-xl">
                    <div className="relative flex-1">
                        ...
                        <input 
                            ...
                            className="w-full pl-12 pr-4 py-3 bg-white border border-slate-200 rounded-lg outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition-all text-slate-700 shadow-sm"
                        />
                    </div>
                    {search && (
                        <button 
                        ...
                        <tbody className="divide-y divide-slate-100 text-slate-600">
                            {filteredOrders.length > 0 ? (
                                filteredOrders.map(order => (
                                    ...
                                    >
                                        <td className="p-5 font-mono text-blue-600 font-bold">{order.trackingNumber}</td>
                                        <td>
                                ...
                                <tr>
                                    <td colSpan={5} className="p-12 text-center text-slate-400 bg-slate-50/50">
                                        <div className="flex flex-col items-center justify-center">
                                            ...
                                            <p className="text-lg font-medium">No expected inbound parcels found.</p>
                                        </div>
                                    </td>
                                </tr>
                            )}
...
export default ExpectedInboundList;
